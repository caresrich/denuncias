...
<h3 style="text-align: center;">REPORTE DE DENUNCIAS POR TIPO DE FALTA</h3>
<span><strong>Denuncias ingresadas desde el : </strong>{{ \Carbon\Carbon::parse($fechaInicio)->format('d/m/Y')}}</span>
<span><strong>al : </strong>{{ \Carbon\Carbon::parse($fechaFin)->format('d/m/Y')}}</span><span style="float: right;"> <strong>Tipo de falta:</strong> {{$falta->nombreFalta}} </span><br>
